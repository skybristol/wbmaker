...
from wikibaseintegrator import WikibaseIntegrator, datatypes, wbi_enums, wbi_login
from wikibaseintegrator.wbi_config import config as wbi_config


class WB:
    def __init__(self, cache_props: bool = True, is_bot: bool = True):
        ...
        if os.environ.get("WB_BOT_USER") and os.environ.get("WB_BOT_PASS"):
            self.login_instance = wbi_login.Login(
                user=os.environ.get("WB_BOT_USER"), password=os.environ.get("WB_BOT_PASS")
            )
            self.wbi = WikibaseIntegrator(login=self.login_instance, is_bot=is_bot)

            self.mw_site.login(
                username=os.environ.get("WB_BOT_USER"), password=os.environ.get("WB_BOT_PASS")
            )
        else:
    ...
    def sparql_query(self, query: str, endpoint: str | None = None, output: str = "dataframe"):
        if not endpoint:
        ...
        if output == "raw":
            ...
            for record in json_results["results"]["bindings"]:
                ...
                for var_name in var_names:
                    data_record[var_name] = (
                        record[var_name]["value"] if var_name in record else None
                    )
            ...
            if output == "dataframe":
                ...
                return (
                    df[["lookup_value", "identifier"]]
                    .set_index("lookup_value")
                    .to_dict()["identifier"]
                )
            else:
                return data_records
    ...
    def wd_path_analysis(
        self, wd_start, wd_lookup, wd_end="Q35120", alternate_path=False, max_paths=10
    ):
        """
        ...
        description_lookup.update(
            df.set_index("next_subclass")["next_subclassDescription"].to_dict()
        )

        G = nx.Graph()

        nodes = list(
            set(df["item"].to_list() + df["subclass_of"].to_list() + df["next_subclass"].to_list())
        )
        G.add_nodes_from(nodes)

        edges = list(df[["item", "subclass_of"]].to_records(index=False)) + list(
            df[["subclass_of", "next_subclass"]].to_records(index=False)
        )
